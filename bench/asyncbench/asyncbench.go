...
type redisTask func(id string, signal chan int, cli *AsynClient, iterations int)
...
func benchTask(taskspec taskSpec, iterations int, workers int, printReport bool) (delta time.Duration, err error) {
	...
	rdc := GetAsynClient("127.0.0.1:6379")
	if rdc == nil {
...
func doSet(id string, signal chan int, cli *AsynClient, cnt int) {
	key := "set-" + id
	value := "foo"
	for i := 0; i < cnt; i++ {
...
func doGet(id string, signal chan int, cli *AsynClient, cnt int) {
	key := "set-" + id
	for i := 0; i < cnt; i++ {
		//	log.Println("get", key)
		_, err := cli.Get(key)
...
func doDel(id string, signal chan int, cli *AsynClient, cnt int) {
	key := "set-" + id
	for i := 0; i < cnt; i++ {
		_, err := cli.Del(key)
		if err != nil {
